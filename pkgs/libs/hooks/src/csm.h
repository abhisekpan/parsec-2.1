...
//Abhi new threadreg to pass core id to pin
static inline unsigned int threadreg_extraparam(unsigned int n, unsigned int c)
{
	...
	//asm volatile ("" ::: "memory");
        asm volatile ("movq %0, %%rax" : : "g" (val) : "rax");
	asm volatile ("movq %0, %%rcx" : : "g" (cval) : "rcx");
        asm volatile ("xchg %bx,%bx");
	//	asm volatile ("movq %%rcx, %0" : "=g" (cval) :  );
        asm volatile ("movq %%rax, %0" : "=g" (val) :  );
	//asm volatile ("" ::: "memory");
        return (unsigned int) val;
}
...
static inline void binding_done(unsigned int c)
{
  threadreg_extraparam(CSM_CODE_DO_FIRSTTOUCH, c);
}
